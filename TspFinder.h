...
public:
	...
	template <typename MASK>
	inline double lowerBound(unsigned start, MASK mask) const {
	...
	template <typename MASK>
	inline void next(const Node<MASK>& node, unsigned added, vector<unsigned>& path) {
		if (m_endLimit < added) {
	...
	template <typename MASK>
	inline void finish(const Node<MASK>& node, unsigned added, vector<unsigned>& path) {
		if (added == m_size) {
